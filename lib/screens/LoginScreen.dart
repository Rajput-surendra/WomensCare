...
class _LoginScreenState extends State<LoginScreen> {
  ...
  loginIntoAccount(int type) async {
    ...
    Response response;
    Dio dio = new Dio();
    print(" this is a login Page${SERVER_ADDRESS +
    "/api/userlogin?login_type=$type&device_token=${await firebaseMessaging
        .getToken()}&device_type=1&email=$email"}");
    ...
    print("parameters here ${SERVER_ADDRESS +
        "/api/userlogin?login_type=$type&device_token=${await firebaseMessaging.getToken()}&device_type=1&email=$email"}  and ${formData.toString()} and  ");
    response = await dio
        .post(
            SERVER_ADDRESS +
    ...
    print('_____ffsfsdf_____${response.data}_________');

    print(response.realUri);
    print("response of login is here" + response.data.toString());
