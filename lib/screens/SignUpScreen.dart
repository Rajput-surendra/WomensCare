...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  createAccount() async {
    if (path.isEmpty) {
      ...
      print(SERVER_ADDRESS + "/api/userregister");
      response =
          await dio.post(SERVER_ADDRESS + "/api/userregister", data: formData);
      if (response.statusCode == 200) {
        var result = jsonDecode(response.data);
        print('${result['status']}____________________');
        ...
        FirebaseDatabase.instance
          ...
          "profile": "profile/" +
              result['data']['profile_pic'].toString().split("/").last,
        }).then((value) async {
          print("\n\nData added in cloud firebase\n\n");
          FirebaseDatabase.instance
              .reference()
              .child(result['data']['user_sid'].toString())
              .child("TokenList")
            ...
            await SharedPreferences.getInstance().then((value) {
              ...
              value.setString("name", result['data']['name']);
              value.setString("email", result['data']['email']);
              value.setString("phone_no", result['data']['phone_no']);
              value.setString("password", password);

              value.setString("profile_pic",
                  result['data']['profile_pic'].toString());
              value.setInt("id", result['data']['id']);
              value.setInt("usertype", result['data']['usertype']);
              value.setInt("uid", result['data']['id']);
            });
